...
def add_product(new_product: NewProduct, response: Response):
    if new_product.name.lower() in [p['name'].lower() for p in products]:
...
# ⭐ BONUS
@app.get('/orders/page')
def orders_page(page: int = Query(1, ge=1), limit: int = Query(3, ge=1, le=20)):
    start = (page - 1) * limit
    end = start + limit
...
def view_cart():
    if not cart:
        return {'message': 'Cart empty', 'items': [], 'total': 0}
...
def checkout(data: CheckoutRequest):
    ...
    return {'message': 'Checkout done', 'orders': placed, 'total': total}
